...
df = df.rename(columns={'country': 'name', 'rating': 'elo'})
...
missing = df[df['group'].isna()]['name'].tolist()
if missing:
...
df['off'] = (BASE_RATE * (1 + 0.5*(df['elo']-mean_elo)/SCALE)).clip(0.3, 3.5)
df['def'] = (BASE_RATE * (1 - 0.5*(df['elo']-mean_elo)/SCALE)).clip(0.3, 3.5)
 
print(f"Dataset: Cruz (2026) Kaggle | Snapshot: 2026-05-27 | Teams: {len(df)}")
print(f"Mean Elo: {mean_elo:.0f} | Range: {df['elo'].min()}–{df['elo'].max()}")
print(f"\nTop 10 by Elo:")
print(df.sort_values('elo', ascending=False)[['name','elo','off','def','group']].head(10).to_string(index=False))
 
groups  = {g: gdf['name'].tolist() for g, gdf in df.groupby('group')}
