...
def load_ratings(path: str, mode: str = 'elo') -> pd.DataFrame:
    ...
    path : str
        ...
        or any CSV with 'name'/'team_name' and 'elo'/'elo_rating' columns.
    mode : str
    ...
    -------
    pd.DataFrame
        Preprocessed ratings with columns: name, elo, off, def
    """
    # Step 1: Load
    df = pd.read_csv(path)
    ...
    df = df[df['name'].isin(QUALIFIED_TEAMS_2026)].reset_index(drop=True)
 
    missing_teams = set(QUALIFIED_TEAMS_2026) - set(df['name'].tolist())
    if missing_teams:
...
def _derive_goal_rates(df: pd.DataFrame) -> pd.DataFrame:
    """
    Derive SPI-style offensive and defensive goal rates from Elo ratings.
 
    Uses logistic normalization relative to the field mean:
        off_i = BASE_RATE * (1 + 0.5 * (elo_i - mean_elo) / SCALE)
        def_i = BASE_RATE * (1 - 0.5 * (elo_i - mean_elo) / SCALE)
    ...
    df['off'] = BASE_RATE * (1 + 0.5 * (df['elo'] - mean_elo) / SCALE)
    df['def'] = BASE_RATE * (1 - 0.5 * (df['elo'] - mean_elo) / SCALE)
...
def _validate(df: pd.DataFrame) -> None:
    ...
    print(f"Dataset validated: {len(df)} teams | "
          f"Elo range: {df['elo'].min():.0f}–{df['elo'].max():.0f} | "
